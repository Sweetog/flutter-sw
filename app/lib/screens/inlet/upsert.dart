...
/// The _job!.inlets![editIndex!] assumption:
/// alot of assumptions made here but the integrity is good:
/// - the job has been retrieved in initData
/// - the job has inlets because argument editIndex is not null
class InletUpsert extends StatefulWidget {
  final String _jobId;
  final int? editIndex;

  InletUpsert(this._jobId, {this.editIndex});

  @override
  _InletUpsertState createState() =>
      _InletUpsertState(this._jobId, editIndex: this.editIndex);
}

class _InletUpsertState extends State<InletUpsert> {
  ...
  @override
  _InletUpsertState(this._jobId, {this.editIndex});
  ...
  String? _getBeforeImgPath() {
    ...
    if (_beforeImg == null && editIndex != null) {
      return _job!.inlets![editIndex!].beforeImgUrl;
    }
  ...
  String? _getAfterImgPath() {
    ...
    if (_afterImg == null && editIndex != null) {
      return _job!.inlets![editIndex!].afterImgUrl;
    }
  ...
  Future<void> _initData() async {
    try {
      ...
      if (editIndex != null) {
        _job = await JobService.getJob(_jobId);
        var editInlet = _job!.inlets![editIndex!];
  ...
  Future<void> _createInlet() async {
    String beforeImgUrl = await StorageUtil.uploadInlet(
        File(_beforeImg!.path), 'before-$_jobId-${_beforeImg!.name}');

    String? afterImgUrl;
    if (_afterImg != null)
      afterImgUrl = await StorageUtil.uploadInlet(
          File(_afterImg!.path), 'after-$_jobId-${_afterImg!.name}');

    var model = _instantiateInletModel(beforeImgUrl, afterImgUrl,
        _currentPosition!.latitude, _currentPosition!.longitude);
    await JobService.createInlet(jobId: _jobId, model: model);
  }

  Future<void> _updateInlet() async {
    InletModel editModel = _job!.inlets![editIndex!];
    String beforeImgUrl;
    String? afterImgUrl;
    ...
    if (_afterImg == null) {
      ...
      afterImgUrl = await StorageUtil.uploadInlet(
          File(_afterImg!.path), 'before-$_jobId-${_afterImg?.name}');
    }
    ...
    await JobService.updateInlet(
        jobId: _jobId, model: model, inletIndex: editIndex!);
  }
  ...
  Future<void> _showChoiceDialog(BuildContext context, bool isBefore) {
    return showDialog(
        context: context,
  ...
  void _openGallery(BuildContext context, bool isBefore) async {
    final pickedFile = await ImagePicker().pickImage(
      source: ImageSource.gallery,
    );
    setState(() {
      if (isBefore) {
        this._beforeImg = pickedFile;
      } else {
        this._afterImg = pickedFile;
      }
    });
  ...
  void _openCamera(BuildContext context, bool isBefore) async {
    final pickedFile = await ImagePicker().pickImage(
